...
export function calculateTopUpSIP(startingSIP, annualTopUpPercent, annualReturnRate, years) {
  ...
  // Year-wise compounding: each year the SIP increases
  for (let y = 1; y <= years; y++) {
    const currentSIP = startingSIP * Math.pow(1 + annualTopUpPercent / 100, y - 1);
    const monthsRemaining = (years - y) * 12 + 12; // months from start of this year to end

    // This year's 12 monthly SIPs compounded to end of investment horizon
    let yearContribution = 0;
    for (let m = 1; m <= 12; m++) {
      const monthsToEnd = (years - y) * 12 + (12 - m + 1);
      yearContribution += currentSIP * Math.pow(1 + r, monthsToEnd);
    }

    totalInvested += currentSIP * 12;
    totalValue += yearContribution;
  ...
  // Recalculate totalValue from yearlyData to get accumulated
  // Actually let's recalculate properly with a running sum
  let runningValue = 0;
  ...
  for (let y = 1; y <= years; y++) {
    ...
    // Value at end of year y: sum of all monthly contributions compounded
    let fv = 0;
    let monthCounter = 0;
    for (let yr = 1; yr <= y; yr++) {
      ...
      for (let m = 1; m <= 12; m++) {
        monthCounter++;
        const monthsToEnd = y * 12 - monthCounter;
        fv += sip * Math.pow(1 + r, monthsToEnd) * (1 + r);
      }
    }
  ...
  const finalValue = correctedData.length ? correctedData[correctedData.length - 1].value : 0;
  const finalInvested = correctedData.length ? correctedData[correctedData.length - 1].invested : 0;

  return {
    totalInvested: finalInvested,
    futureValue: finalValue,
    wealthGained: finalValue - finalInvested,
    yearlyData: correctedData,
  };
}
